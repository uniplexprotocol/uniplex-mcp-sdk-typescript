...
exports.verifySignatureSync = verifySignatureSync;
exports.validateConstraints = validateConstraints;
exports.mergeConstraints = mergeConstraints;
exports.verifyLocally = verifyLocally;
exports.buildPassportIndex = buildPassportIndex;
exports.hasPermission = hasPermission;
exports.getPermissionConstraints = getPermissionConstraints;
const ed = __importStar(require("@noble/ed25519"));
const types_js_1 = require("./types.js");
class InMemoryRateLimiter {
    buckets = new Map();
    limits = new Map();
    setLimit(action, max, windowMs = 60000) {
        this.limits.set(action, { max, windowMs });
    }
    getBucketKey(action, passportId) {
        return passportId ? `${action}:${passportId}` : action;
    }
    check(action, passportId) {
 ...
 */
async function verifySignature(passport, issuerPublicKey) {
    try {
 ...
 */
function verifySignatureSync(passport, issuerPublicKey) {
    try {
        const signedPayload = {
            passport_id: passport.passport_id,
            issuer_id: passport.issuer_id,
 ...
 */
function validateConstraints(passportConstraints, context, catalogConstraints) {
    for (const [key, passportValue] of Object.entries(passportConstraints)) {
        const contextValue = context[key];
        // Skip if no context value for this constraint
        if (contextValue === undefined)
            continue;
        const constraintType = types_js_1.CONSTRAINT_TYPES[key];
 ...
 */
function mergeConstraints(catalogConstraints, passportConstraints) {
    const merged = { ...catalogConstraints };
    for (const [key, passportValue] of Object.entries(passportConstraints)) {
        const constraintType = types_js_1.CONSTRAINT_TYPES[key];
        if (constraintType?.type === 'limit' && typeof passportValue === 'number') {
...
// =============================================================================
function resolveCatalogVersion(catalog, passport) {
...
function verifyLocally(params) {
    ...
    const constraintResult = validateConstraints(passportPermission.constraints, context, catalogEntry.constraints);
    ...
    return {
        allowed: true,
        effective_constraints: mergeConstraints(catalogEntry.constraints, passportPermission.constraints),
        confident: true,
    };
}
// =============================================================================
 ...
 */
function buildPassportIndex(passport) {
    const claimsByKey = {};
 ...
 */
function hasPermission(passport, action) {
    if (!passport)
        return false;
 ...
 */
function getPermissionConstraints(passport, action) {
    if (!passport)
        return undefined;
