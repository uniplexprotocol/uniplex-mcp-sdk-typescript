...
class SessionManager {
    ...
    // SESSION CRUD
    // ==========================================================================
    async getOrCreateSession(sessionId, meta) {
    ...
    // PASSPORT MANAGEMENT
    // ==========================================================================
    setPassport(sessionId, passport) {
        const session = this.sessions.get(sessionId);
        if (session) {
            // Ensure claimsByKey is built
            session.passport = (0, verification_js_1.buildPassportIndex)(passport);
            session.last_activity = Date.now();
        }
    }
    getPassport(sessionId) {
        return this.sessions.get(sessionId)?.passport ?? null;
    }
    clearPassport(sessionId) {
        const session = this.sessions.get(sessionId);
        if (session) {
    ...
    // ==========================================================================
    sessionHasPermission(sessionId, action) {
        const passport = this.getPassport(sessionId);
        return (0, verification_js_1.hasPermission)(passport, action);
    }
    ...
    getSessionConstraints(sessionId, action) {
        const passport = this.getPassport(sessionId);
        if (!passport)
            return undefined;
        if (action) {
            return (0, verification_js_1.getPermissionConstraints)(passport, action);
        }
     ...
     */
    async issueSafeDefault(agentId, issuerId) {
        const response = await fetch(`${this.apiUrl}/gates/${this.gateId}/passports/safe-default`, {
            method: 'POST',
            headers: {
        ...
        return (0, verification_js_1.buildPassportIndex)(data.passport);
    }
    // ==========================================================================
    // SESSION CLEANUP
    // ==========================================================================
...
class SessionWrapper {
    ...
    hasPermission(action) {
        return (0, verification_js_1.hasPermission)(this.session.passport, action);
    }
    getPermissions() {
    ...
    getConstraints(action) {
        if (!this.session.passport)
            return undefined;
        if (action) {
            return (0, verification_js_1.getPermissionConstraints)(this.session.passport, action);
        }
 ...
 */
function createTestSession(sessionId, permissions, constraints = {}) {
    const now = new Date();
    ...
    return {
        session_id: sessionId,
        passport: (0, verification_js_1.buildPassportIndex)(mockPassport),
